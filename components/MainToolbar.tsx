...
const ToolbarButton: React.FC<{
    ...
    showDot?: boolean;
    className?: string;
}> = ({ onClick, icon, title, isActive, activeColor = "text-white", activeBg, activeBorder, showDot, className = "w-12" }) => {
    ...
    return (
        <button
            ...
            title={title}
        >
            <span className={`material-symbols-outlined text-base ${isActive ? activeColor : "text-white"}`}>
                {icon}
            </span>
            {showDot && (
                <span className={`ml-2 w-1.5 h-1.5 rounded-full ${activeColor.replace('text-', 'bg-')} ${activeColor} shadow-[0_0_8px_currentColor]`} />
            )}
        </button>
    );
};
