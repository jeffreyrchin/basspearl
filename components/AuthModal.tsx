...
const AuthModal = () => {
    ...
    return (
        <div className="fixed inset-0 z-auth-modal flex items-center justify-center p-4">
            {/* Backdrop */}
            <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
            ...
            <motion.div
                ...
                data-section="modal"
                className="relative w-full max-w-md glass-panel rounded-lg border border-white/10 max-h-[90vh] overflow-hidden flex flex-col shadow-2xl"
            >
                {/* Header Gradient */}
                <div className="absolute top-0 left-0 right-0 h-32 bg-gradient-to-b from-primary/60 to-transparent pointer-events-none z-0" />

                {/* Close Button */}
                <button
                    onClick={closeAuth}
                    className="absolute top-4 right-4 size-10 rounded-full bg-white/5 hover:bg-white/10 flex items-center justify-center transition-colors z-20"
                >
                    <span className="material-symbols-outlined text-white/60">close</span>
                </button>

                {/* Scrollable Content */}
                <div className="flex-1 overflow-y-auto custom-scrollbar z-10 overscroll-contain">
                    <div className="relative p-8 pt-12">
                        ...
                        <form onSubmit={handleEmailSubmit} className="space-y-4">
                            ...
                            <button
                                ...
                                className={`w-full py-3.5 rounded-xl bg-primary/10 border border-primary hover:bg-primary/30 text-white font-bold text-sm uppercase tracking-widest transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 ${authMode === 'signup' && !agreedToTerms ? 'opacity-50 grayscale' : ''}`}
                            >
                                {isLoading ? (
                                    <>
                        ...
                        <p className="text-center text-white/60 text-sm mt-6">
                            ...
                            <button
                                onClick={toggleMode}
                                className="ml-2 text-primary hover:text-white font-bold uppercase tracking-wider transition-colors"
                            >
